...
def smile_to_graph(smile):
    ...
    mol_size = mol.GetNumAtoms()  # 原子数量

    mol_features = []
    for atom in mol.GetAtoms():  # 获取原子特征
        feature = atom_features(atom)  # 原子特征
        mol_features.append(feature / sum(feature))  # 原子特征进行正则化

    edges = []
    for bond in mol.GetBonds():  # 创建边索引
    ...
    if len(edge_index) == 0:
        edge_index = [[0, 0]]  # 默认的边索引

    return mol_size, mol_features, edge_index  # 返回原子数量，节点特征，边索引
...
def create_DTA_dataset(dataset, fold):
    # load protein feature and predicted distance map
    process_dir = os.path.join('pre_process')
    pro_distance_dir = os.path.join(process_dir, 'sequence_representations_parasite')  # numpy .npy file
    # drug smiles
    df_ligands = pd.read_csv('data/' + dataset + '/duplicates_smiles.csv')

    # protein sequences
    df_proteins = pd.read_csv('data/' + dataset + '/duplicates_sequence.csv')
    ...
    for index, row in df_ligands.iterrows():
        drugs.append(row['Molecule ChEMBL ID'])
        drug_smiles.append(row['Smiles'])
    ...
    for index, row in df_proteins.iterrows():
        ...
        prot_keys.append(row['UniProt Accessions'])
    ...
    # 处理药物和蛋白质的编码向量
    df_smiles_emd = pd.read_csv('data/' + dataset + '/duplicates_smiles_embedding.csv')
    df_fasta_emd = pd.read_csv('data/' + dataset + '/duplicates_sequence_embedding.csv')

    drug_emb_dic = {}
    fasta_emb_dic = {}

    # 获取药物嵌入字典
    for index, row in df_smiles_emd.iterrows():
        # 将字符串转换为 numpy 数组并确保其为 np.float32 类型
        drug_emb_dic[row['Smiles']] = np.asarray(str_to_np_array(row['Smiles_vector']), dtype=np.float32)


    # 获取蛋白质嵌入字典
    for index, row in df_fasta_emd.iterrows():
        fasta_emb_dic[row['UniProt Accessions']] = np.asarray(str_to_np_array(row['Sequence_vector']), dtype=np.float32)

    train_csv = 'data/' + dataset + '/' + fold + '/data_train.csv'
    val_csv = 'data/' + dataset + '/' + fold + '/data_valid.csv'
    test_csv = 'data/' + dataset + '/' + fold + '/data_test.csv'
    df_train = pd.read_csv(train_csv)
    df_val = pd.read_csv(val_csv)
    df_test = pd.read_csv(test_csv)

    train_drugs, train_prots, train_target_key, train_Y = list(df_train['Smiles']), list(df_train['Sequence']), list(
        df_train['UniProt Accessions']), list(df_train['Average_pIC50'])
    val_drugs, val_prots, val_target_key, val_Y = list(df_val['Smiles']), list(df_val['Sequence']), list(
        df_val['UniProt Accessions']), list(df_val['Average_pIC50'])
    test_drugs, test_prots, test_target_key, test_Y = list(df_test['Smiles']), list(df_test['Sequence']), list(
        df_test['UniProt Accessions']), list(df_test['Average_pIC50'])

    train_T = [seq_cat(t) for t in train_prots]  # 蛋白质序列数值化
    val_T = [seq_cat(t) for t in val_prots]  # 蛋白质序列数值化
    test_T = [seq_cat(t) for t in test_prots]  # 蛋白质序列数值化

    train_D = [smiles_cat(t) for t in train_drugs]  # 药物序列数值化
    train_D = np.asarray(train_D)
    val_D = [smiles_cat(t) for t in val_drugs]  # 药物序列数值化
    val_D = np.asarray(val_D)
    test_D = [smiles_cat(t) for t in test_drugs]  # 药物序列数值化
    test_D = np.asarray(test_D)

    # 将数据转换为numpy数组
    train_drugs, train_prots, train_target_key, train_Y = np.asarray(train_drugs), np.asarray(train_T), np.asarray(
        train_target_key), np.asarray(train_Y)
    val_drugs, val_prots, val_target_key, val_Y = np.asarray(val_drugs), np.asarray(val_T), np.asarray(
        val_target_key), np.asarray(val_Y)
    test_drugs, test_prots, test_target_key, test_Y = np.asarray(test_drugs), np.asarray(test_T), np.asarray(
        test_target_key), np.asarray(test_Y)

    train_data = DTADataset(root='data', dataset=dataset + 'data_train', xd=train_drugs, smi_char=train_D,
                            xt_char=train_prots, smi_emb=drug_emb_dic, fas_emb=fasta_emb_dic, y=train_Y,
                            target_key=train_target_key, smile_graph=smile_graph,
                            target_graph=target_graph)

    val_data = DTADataset(root='data', dataset=dataset + 'data_valid', xd=val_drugs, smi_char=val_D, xt_char=val_prots,
                          smi_emb=drug_emb_dic, fas_emb=fasta_emb_dic, y=val_Y,
                          target_key=val_target_key, smile_graph=smile_graph,
                          target_graph=target_graph)

    test_data = DTADataset(root='data', dataset=dataset + 'data_test', xd=test_drugs, smi_char=test_D,
                           xt_char=test_prots, smi_emb=drug_emb_dic, fas_emb=fasta_emb_dic, y=test_Y,
                           target_key=test_target_key, smile_graph=smile_graph,
                           target_graph=target_graph)
